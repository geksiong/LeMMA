...
class Application(Frame):
	...
	def doSaveMMA(self, filename):
		...
		f = open(filename, "w")
		i = 0	# measure number
		print >>f, "// MMA file generated by LeMMA version " + version + "\n"
		print >>f, "KeySig " + self.keyEntry.get()
		print >>f, "TimeSig " + self.timeSigEntry.get().replace("/", " ")
		print >>f, "Tempo " + self.tempoEntry.get() + "\n"
		if self.SwingMode.get() == 1:
			print >>f, "SwingMode On\n"
		else:
			print >>f, "SwingMode Off\n"
		...
		for measure in self.measures:
			...
			if contents != "":
				...
				if isRepeat == 1:
					print >>f, "Repeat"
				if isRepeatEnding == 1:
					print >>f, "RepeatEnding"
				
				print >>f, str(i+1) + " " + contents
